...
export default function HandleReceipt() {
    ...
    const paramValueR = searchParam.get('id');
    ...
    function handleSave() {
        ...
        if (paramValueR) {
            axios.post('http://localhost:3001/updateReceipt', {
                id: paramValueR,
                name: name,
                date: date,
            ...
            .then(result => {
                ...
                window.location.href = '/viewReceipt';
            })
            .catch(err => console.log(err));
        } else {
            axios.post('http://localhost:3001/receipt', {
                name: name,
                date: date,
                particulars: particulars,
                amount: amount,
    ...
    return (
        <div className="w-[35rem] mt-16 ml-80">
            <menu className="flex flex-col item-center justify-center gap-4 my-4 px-4 bg-stone-200">
                <h2 className='flex items-center justify-center uppercase text-stone-600 font-bold mt-4'>record your receipt</h2>
                <div className="flex flex-col w-full p-1 rounded-sm border-stone-300 bg-stone-200 text-stone-600 focus:outline-none focus:border-stone-600 mb-4">
                    ...
                    {nameWarning && <span className="text-red-500">Please fill out your name.</span>}
                    <Input label='Date' type='date' value={date} onChange={setDate} />
                    {dateWarning && <span className="text-red-500">Please select a date.</span>}
                    ...
                    {amountWarning && <span className="text-red-500">Please fill out this field with a valid Amount.</span>}
                </div>
